...
public class ActivityController {
    ...
    //但在进入编辑页面是作为参数回传到后台.
    public List geActivityMapList(String numName) {
        ...
        for(int i = 0; i < sList.size();i++) {
            ...
            m.put("dates",DateTimeTool.parseDateTime(s.getDates(),"yyyy-MM-dd"));
    ...
    public DataResponse activityInit(@Valid @RequestBody DataRequest dataRequest) {
        List dataList = geActivityMapList("");
    ...
    public DataResponse activityQuery(@Valid @RequestBody DataRequest dataRequest) {
        ...
        List dataList = geActivityMapList(numName);
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }

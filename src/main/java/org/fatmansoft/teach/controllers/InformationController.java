...
public class InformationController {
    ...
    @PostMapping("/informationInit")
    @PreAuthorize("hasRole('ADMIN')")
    public DataResponse informationInit(@Valid @RequestBody DataRequest dataRequest) {
        List dataList = getInformationMapList("");//传递空串以遍历出所有的数据
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }
    //information页面点击查询按钮请求
    //Table界面初始是请求列表的数据，从请求对象里获得前端界面输入的字符串，作为参数传递给方法getInformationMapList，返回所有学生数据，
